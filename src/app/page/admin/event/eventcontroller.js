(function() {
    ...
    function EventController(AdminService, $mdDialog, $filter, triTheming, $mdToast, triLayout, SignUpService) {
        ...
        function getAllMeeting() {
            AdminService.getActivity().then(function(response) {
                ...
                for (var i = 0; i <= vm.meetings.length; i++) {
                    console.log(vm.meetings[i].location);
        ...
        function save() {
           ...
           AdminService.createActivity(vm.event.subject, vm.event.numOfPe, vm.event.startDate, vm.event.endDate, vm.event.content, vm.event.location, vm.event.school)
           $mdDialog.show(
                $mdDialog.alert()
